...
export default function AdminPage() {
    ...
    try {
      ...
      reader.onload = (event) => {
        ...
        const newImports = imports.filter(
          (importItem) =>
            !items?.some((item) => item.marketDef == importItem.name)
        );
        ...
        for (const importItem of newImports) {
          ...
          const typeMatch = importItem.enUiType
            ...
            .replace(/[^0-9A-Za-z_-]/g, "");
          const newType =
            types?.find((type) => type.name.match(typeMatch))?.id ?? 0;
          const newRarity =
            rarities?.find((rarity) => rarity.importKey == importItem.rarity)
              ?.id ?? 0;
  ...
  return (
    <div className="w-full max-w-3xl px-3 mx-auto">
      <h1 className="mb-10 text-3xl font-bold text-gray-900">Upload file</h1>
      ...
      >
        <div className="flex flex-col md:flex-row gap-1.5 md:py-4">
          <label className="flex flex-col items-center justify-center flex-grow h-full py-3 transition-colors duration-150 cursor-pointer hover:text-gray-600">
            ...
            <strong className="text-sm font-medium">Select json file</strong>
            <input
      ...
      <Table dataSource={tableData} rowKey="marketDef">
        <Table.Column dataIndex="id" title={translate("item.fields.id")} />

        <Table.Column dataIndex="name" title={translate("item.fields.name")} />
        <Table.Column
          dataIndex="quantity"
